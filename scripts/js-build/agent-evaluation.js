...
const EVALUATOR_ADDRESS = "agent1qw254tc8q3mcmrseem0pmhu2jd0j7urn2e9cd5tgcg88kmy9wkqhysksdwf";
async function evaluateAgent(address, category = "good") {
 ...
 */
function generateEvaluationScore(address, category) {
    ...
    const correctnessEffectiveScore = Math.round((correctnessScore * correctnessConfidence) / 10);
    const capabilitiesEffectiveScore = Math.round((capabilitiesScore * capabilitiesConfidence) / 10);
    const domainEffectiveScore = Math.round((domainScore * domainConfidence) / 10);
    // Calculate final weighted score
    const finalScore = Math.round((correctnessEffectiveScore * correctnessWeight +
    ...
    const overallConfidence = Math.round((correctnessConfidence + capabilitiesConfidence + domainConfidence) / 3);
 ...
 */
function randomInRange(min, max, useNormal = true) {
    if (!useNormal) {
 ...
 */
function generateScore(category) {
    switch (category) {
        case "good":
